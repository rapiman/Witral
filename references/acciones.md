...
Tools disponibles hoy: `git_clone`, `git_init`, `git_status`, `git_log`, `git_diff`, `git_branch`,
...
- **Traer un repo:** `git_clone(url, destino, rama, donde)` clona `url` en `destino`. A
  diferencia del resto, el repo destino aún no existe, así que no usa `git -C`: pasa el
  destino como argumento. En local el destino se acota a la raíz autorizada; en remoto se
  interpreta en ese lugar. `rama` opcional clona solo esa rama (`--branch`). Es solo
  descarga (no publica ni destruye), así que no pide confirmación, igual que `pull`/`fetch`.
